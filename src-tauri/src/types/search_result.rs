...
impl SearchResult {
    pub fn from_html(
        ...
        Self::from_html_with_download_dir(Some(download_dir.as_path()), html, is_search_by_tag)
    }

    pub fn from_html_with_download_dir(
        ...
        for comic_li in document.select(&comic_li_selector) {
            let comic = ComicInSearch::from_li_with_download_dir(download_dir, &comic_li)?;
            comics.push(comic);
        }
...
impl ComicInSearch {
    ...
    pub fn from_li_with_download_dir(
        ...
        let is_downloaded = download_dir.is_some_and(|dir| dir.join(&title).exists());
